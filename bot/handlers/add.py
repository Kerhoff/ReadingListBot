...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    tg_username: str = str(update.effective_user.username)
    ...
    item_type: str = user_input[-1].lower().replace("#", "")
    if item_type not in ["book", "article", "video", "podcast"]:
    ...
    await context.bot.send_message(chat_id=chat_id, text=messages.ITEM_ADDED)
