...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(user_input) < 1:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=messages.ADD_ITEM_USAGE
        )
        return
    ...
    if item_type not in ["book", "article", "post", "video", "podcast"]:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=messages.INVALID_ITEM_TYPE
        )
        return
    ...
    if not re.match(r"^https?://", link):
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=messages.INVALID_URL
        )
        return
    ...
    add_item(
        user_id=update.effective_user.id, link=link, title=title, item_type=item_type
    )
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text=messages.ITEM_ADDED.format(title=title)
    )
